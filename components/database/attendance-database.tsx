...
interface AttendanceDatabaseProps {
...
export function AttendanceDatabase({ employees }: AttendanceDatabaseProps) {
  ...
  const handleExport = async () => {
    ...
    downloadCsv(
      rows.map((row: Record<string, unknown>) => ({
        ...row,
        employee_name: employeeMap[row.employee_id as string] ?? "-",
        hours: calcHours(
          row.check_in as string | null,
          row.check_out as string | null,
          row.break_minutes as number | null
        )
      })),
      [
        ...
        { key: "check_out", label: "Salida" },
        { key: "break_minutes", label: "Descanso (min)" },
        ...
        <div className="space-y-3 md:hidden">
          {loading
            ...
            : data.length === 0
              ...
              : data.map((record) => {
                  const statusInfo = statusMeta.get(record.status ?? "");
